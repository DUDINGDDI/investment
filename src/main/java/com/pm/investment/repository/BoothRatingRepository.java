...
public interface BoothRatingRepository extends JpaRepository<BoothRating, Long> {

    Optional<BoothRating> findByUserIdAndBoothId(Long userId, Long boothId);

    boolean existsByUserIdAndBoothId(Long userId, Long boothId);

    @Query("SELECT r.booth.id FROM BoothRating r WHERE r.user.id = :userId")
    List<Long> findRatedBoothIdsByUserId(@Param("userId") Long userId);

    List<BoothRating> findByBoothIdAndReviewIsNotNullOrderByUpdatedAtDesc(Long boothId);
    ...
    @Query("SELECT r.booth.id, " +
           ...
           "FROM BoothRating r GROUP BY r.booth.id")
